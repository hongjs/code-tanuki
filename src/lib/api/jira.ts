...
export class JiraClient {
   ...
   *
   * Also drops empty `attrs: {}` and empty `marks: []` arrays — Jira's strict
   * validator rejects empty attr objects when the schema expects either a
   * populated attrs map or no attrs key at all.
   */
  private cleanADF(node: any): any {
    ...
    for (const [key, value] of Object.entries(node)) {
      ...
      if (key === 'attrs' && typeof value === 'object' && value !== null) {
        ...
        // For non-table nodes: strip null-valued keys from attrs (e.g. codeBlock with no language
        // gives { language: null }; Jira rejects null attrs as INVALID_INPUT). Recursively clean
        // remaining values, then drop empty attrs entirely.
        const attrsObj = value as Record<string, unknown>;
        const filteredAttrs: Record<string, any> = {};
        for (const [ak, av] of Object.entries(attrsObj)) {
          if (av === null || av === '') continue;
          filteredAttrs[ak] = this.cleanADF(av);
        }
        if (Object.keys(filteredAttrs).length > 0) {
          cleaned.attrs = filteredAttrs;
        }
        continue;
      }

      // Drop empty marks arrays — Jira validator can reject `marks: []`
